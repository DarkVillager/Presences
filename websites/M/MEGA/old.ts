if (privacy) presenceData.details = "Browsing";
	...
	if (pathname === "/" || pathname.includes("/startpage"))
		...
		const extension = document.querySelector("[class='extension']").textContent;
		...
		const selected =
			document.querySelector('[class*="ui-selected"]')?.getAttribute("title") ??
			document
		...
		if (selected) {
			...
			presenceData.details = `Viewing ${
				document.querySelector(".btn-galleries.js-lpbtn.active")?.textContent ??
	...
	else if (pathname === "/business") presenceData.details = "Viewing Business";
}
